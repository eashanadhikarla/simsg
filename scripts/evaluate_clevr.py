...
CLEVR_DIR = os.path.expanduser('./datasets/clevr/target')
SPLIT = "test"
...
#   graph_json_dir = makedir(output_dir, 'graphs_json')
  img_dir = makedir(output_dir, 'images_64_' + SPLIT)
  graph_json_dir = makedir(output_dir, 'graphs_json_64')
  ...
  for batch in loader:
    ...
    model_out = model(objs, triples_, obj_to_img,
        ...
        query_feats=query_feats, keep_box_idx=keep_box_idx, keep_feat_idx=keep_feat_idx,keep_image_idx=keep_image_idx)
    ...
    for s in range(imgs.shape[0]):
      ...
      image1 = (imgs[s:s + 1, :, :, :] / 255.0).cuda()
      image2 = (imgs_pred[s:s + 1, :, :, :] / 255.0).cuda()
      #.cpu()
      curr_ssim = pytorch_ssim.ssim(image1,image2, window_size=3).cpu().item()
#       curr_ssim_loss = pytorch_ssim.ssim(window_size=3).cpu().item()
#       curr_ssim = curr_ssim_loss(imgs[s:s + 1, :, :, :] / 255.0,
#                           imgs_pred[s:s + 1, :, :, :] / 255.0)
      ssim_per_image_all.append(curr_ssim)
      ssim_per_image[mode].append(curr_ssim)
      img1 = (imgs[s:s + 1, :, top:bottom, left:right] / 255.0).cuda()
      img2 = (imgs_pred[s:s + 1, :, top:bottom, left:right] / 255.0).cuda()
      curr_ssim_roi = pytorch_ssim.ssim(img1,
                          img2, window_size=3).cpu().item()
    # curr_ssim_roi_loss = pytorch_ssim.ssim(window_size=3).cpu().item()
    # curr_ssim_rol = curr_ssim_roi_loss(imgs[s:s + 1, :, top:bottom, left:right] / 255.0,
