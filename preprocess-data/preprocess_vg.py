...
parser.add_argument('--attributes_json', default=os.path.join(VG_DIR, 'attributes.json'))
parser.add_argument('--object_aliases', default=os.path.join(VG_DIR, 'object_alias.txt'))
parser.add_argument('--relationship_aliases', default=os.path.join(VG_DIR, 'relationship_alias.txt'))
parser.add_argument('--relationships_json', default=os.path.join(VG_DIR, 'relationships.json'))
...
def main(args):
  ...
  for split_name, split_arrays in numpy_arrays.items():
    ...
    with h5py.File(h5_path, 'w') as h5_file:
      ...
      path_dset = h5_file.create_dataset('image_paths', path_shape, dtype=path_dtype)
...
def remove_small_images(args, image_id_to_image, splits):
  ...
  for split_name, image_ids in splits.items():
    ...
    print('Removed %d images from split "%s" for being too small' % (num_skipped, split_name))
